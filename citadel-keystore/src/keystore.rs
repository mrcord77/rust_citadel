...
///
/// **Limitations:** does not persist across restarts. A replay that spans a
/// server restart is not caught. Persistent protection requires a durable
/// nonce store (database, Redis, etc.) — tracked as a future improvement.
struct ReplayCache {
...
impl Keystore {
    ...
    fn wrap_secret_key(&self, key_id: &KeyId, version: u32, sk_bytes: &[u8]) -> SecretKeyMaterial {
        let Some(ref mk) = self.master_key else {
            // Only reachable when CITADEL_ALLOW_PLAINTEXT_KEYS=1 is set explicitly,
            // because generate()/rotate() fail-closed before calling this otherwise.
            tracing::warn!(
                key = %key_id,
                version,
                "Dev mode: storing secret key as plaintext \
    ...
    pub async fn generate(
        ...
        if self.master_key.is_none()
            && std::env::var("CITADEL_ALLOW_PLAINTEXT_KEYS").as_deref() != Ok("1")
        {
            return Err(GenerateError(KeystoreError::StorageError(
                "CITADEL_MASTER_KEY is not set or is invalid. Refusing to store \
                 secret key material as plaintext. Set CITADEL_MASTER_KEY to a \
                 64-char hex string in production, or set \
                 CITADEL_ALLOW_PLAINTEXT_KEYS=1 for local development only."
                    .into(),
            )));
        }
        // ─────────────────────────────────────────────────────────────────────
    ...
    pub async fn rotate(&self, id: &KeyId) -> Result<KeyId, RotateError> {
            ...
            && std::env::var("CITADEL_ALLOW_PLAINTEXT_KEYS").as_deref() != Ok("1")
        {
            return Err(RotateError(KeystoreError::StorageError(
                "CITADEL_MASTER_KEY is not set. Refusing to store rotated key \
                 material as plaintext. Set CITADEL_MASTER_KEY or \
                 CITADEL_ALLOW_PLAINTEXT_KEYS=1 (dev only)."
                    .into(),
            )));
        }
        // ─────────────────────────────────────────────────────────────────────
    ...
    pub async fn destroy(&self, id: &KeyId) -> Result<(), LifecycleError> {
        ...
        // Note: this clears in-process memory. On-disk forensic recovery via
        // filesystem journals or backup copies is an operational concern that
        // requires HSM or full-disk encryption at the infrastructure level.
