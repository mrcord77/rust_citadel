...
impl Keystore {
        ...
        match std::env::var("CITADEL_MASTER_KEY") {
            Ok(hex_str) => match hex::decode(hex_str.trim()) {
                Ok(bytes) if bytes.len() == 32 => {
                    ...
                    tracing::info!("CITADEL_MASTER_KEY loaded — secret keys will be encrypted at rest");
                    Some(key)
                }
                Ok(bytes) => {
                    tracing::error!(
    ...
    fn wrap_secret_key(&self, key_id: &KeyId, version: u32, sk_bytes: &[u8]) -> SecretKeyMaterial {
        let Some(ref mk) = self.master_key else {
            // CITADEL_MASTER_KEY is not set — secret key will be stored as plaintext.
            // This is a security violation in production. The API binary enforces
            // master key presence via process::exit(1), but library callers see this path.
            // Log at ERROR so no key generation or rotation can silently downgrade security.
            tracing::error!(
                key = %key_id,
                version,
                "PLAINTEXT KEY STORAGE: CITADEL_MASTER_KEY is not configured. \
                 Secret key for key={} version={} is being stored unencrypted. \
                 Set CITADEL_MASTER_KEY in production. \
                 Set CITADEL_ALLOW_PLAINTEXT_KEYS=1 only for local development.",
                key_id, version
            );
            return SecretKeyMaterial::Plaintext(hex::encode(sk_bytes));
        };

        // Per-key AES-256 wrapping key: HKDF(master_key, info = "citadel-kek-v1:" || id || ":" || ver)
        ...
        SecretKeyMaterial::Encrypted(format!("enc:{}{}", hex::encode(nonce_bytes), hex::encode(ct)))
    }
        ...
        match material {
            ...
            SecretKeyMaterial::Plaintext(hex_str) => {
                if self.master_key.is_some() {
                    tracing::warn!(
                        key = %key_id,
                        version,
                        "secret key was stored without at-rest encryption;                          rotate the key to re-encrypt under the current master key"
                    );
                }
            ...
            SecretKeyMaterial::Encrypted(enc) => {
                ...
                let nonce_bytes = hex::decode(&payload[..24])
                    .map_err(|e| format!("nonce hex decode: {}", e))?;
    ...
    pub async fn generate(
        ...
        let wrap_nonce_hex = match &material {
            SecretKeyMaterial::Encrypted(enc) if enc.len() > 28 => {
                Some(enc[4..28].to_string()) // "enc:" is 4 chars, nonce is 24 hex chars
            }
            _ => None,
        };
