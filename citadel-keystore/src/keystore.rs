...
impl ReplayCache {
    ...
    /// Returns `true` if `nonce` is fresh (not seen within the TTL window).
    /// Returns `false` on replay. Always records fresh nonces.
    fn check_and_insert(&mut self, nonce: [u8; 12]) -> bool {
        let now = Instant::now();
        self.seen
            .retain(|_, seen_at| now.duration_since(*seen_at) < self.ttl);
        if let std::collections::hash_map::Entry::Vacant(e) = self.seen.entry(nonce) {
            e.insert(now);
            true
        } else {
            false
...
impl Keystore {
    ...
    pub async fn generate(
        ...
        if self.master_key.is_none() {
            let dev_mode = std::env::var("CITADEL_ALLOW_PLAINTEXT_KEYS").as_deref()
                == Ok("1")
    ...
    pub async fn decrypt(
        ...
        // Replay protection: extract the wire-format nonce and reject if seen recently.
        // Wire layout (frozen per PRD): [6 header][1120 KEM ct][12 nonce][AEAD ct]
        // These offsets must stay in sync with citadel-envelope/src/wire.rs constants.
        {
            ...
            nonce.copy_from_slice(&ciphertext[NONCE_OFFSET..NONCE_OFFSET + NONCE_LEN]);
