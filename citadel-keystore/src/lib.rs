...
mod tests {
    ...
    fn test_keystore() -> Keystore {
        // Tests use in-memory storage with no real key material at risk.
        // Both flags required: CITADEL_ALLOW_PLAINTEXT_KEYS=1 AND CITADEL_ENV=development.
    ...
    async fn test_generate_all_key_types() {
        ...
        for kt in [
            ...
            let id = ks
                .generate(format!("{:?}", kt), kt, None, None)
                .await
                .unwrap();
            let meta = ks.get(&id).await.unwrap();
    ...
    async fn test_generate_with_parent() {
        let ks = test_keystore();
        let parent = ks
            .generate("parent", KeyType::KeyEncrypting, None, None)
            .await
            .unwrap();
        let child = ks
            .generate("child", KeyType::DataEncrypting, None, Some(parent.clone()))
            .await
            .unwrap();
    ...
    async fn test_destroy_revoked_key() {
        ...
        assert!(
            meta.versions[0].secret_key_material.is_destroyed(),
            "key material should be in Destroyed state after destroy()"
        );
    ...
    async fn test_policy_compliant() {
        ...
        let id = ks
            .generate("key", KeyType::DataEncrypting, Some(pid), None)
            .await
            .unwrap();
        ks.activate(&id).await.unwrap();
    ...
    async fn test_audit_tracks_encryption() {
        ...
        let has_encrypt = events.iter().any(|e| {
            matches!(
                e.action,
                crate::audit::AuditAction::EncryptionPerformed { .. }
            )
        });
        assert!(has_encrypt);
    }
    ...
    async fn test_list_keys() {
        let ks = test_keystore();
        for i in 0..5 {
            ks.generate(format!("key-{}", i), KeyType::DataEncrypting, None, None)
                .await
                .unwrap();
        }
    ...
    async fn test_list_by_state() {
        let ks = test_keystore();
        let id1 = ks
            .generate("key1", KeyType::DataEncrypting, None, None)
            .await
            .unwrap();
        let id2 = ks
            .generate("key2", KeyType::DataEncrypting, None, None)
            .await
            .unwrap();
        let _id3 = ks
            .generate("key3", KeyType::DataEncrypting, None, None)
            .await
            .unwrap();
    ...
    async fn test_full_lifecycle() {
        let ks = test_keystore();
        let id = ks
            .generate("lifecycle-key", KeyType::DataEncrypting, None, None)
            .await
            .unwrap();
        ...
        let ctx = Context::raw(b"ctx");
        let blob = ks
            .encrypt(&id, b"important data", &aad, &ctx)
            .await
            .unwrap();
    ...
    fn test_policy_adapter_critical_compresses_everything() {
        ...
        let expected_grace =
            Duration::from_secs((base.rotation_grace_period.as_secs() as f64 * 0.1) as u64);
        assert_eq!(adapted.rotation_grace_period, expected_grace);

        // Max lifetime should be 25% of original
        let expected_lifetime = base
            .max_lifetime
            .map(|d| Duration::from_secs((d.as_secs() as f64 * 0.25) as u64));
    ...
    async fn test_keystore_record_threat_event() {
        let ks = test_keystore();
        ks.record_threat_event(ThreatEvent::new(ThreatEventKind::DecryptionFailure, 3.0));
    ...
    async fn test_keystore_threat_escalation_tightens_policy() {
        ...
        let base_grace = ks
            .policy_adaptation_summary(&PolicyId::new("default-dek"))
            .unwrap()
            .effective_grace_period;
        ...
        let adapted_grace = ks
            .policy_adaptation_summary(&PolicyId::new("default-dek"))
            .unwrap()
            .effective_grace_period;
        assert!(
            adapted_grace < base_grace,
            "Expected grace period to shrink: base={:?}, adapted={:?}",
            base_grace,
            adapted_grace
        );
    }
