...
impl SecretKeyMaterial {
    ...
    pub fn is_encrypted(&self) -> bool { matches!(self, Self::Encrypted(_)) }
    /// Returns `true` if the material is wrapped (alias for `is_encrypted`).
    pub fn is_wrapped(&self) -> bool { self.is_encrypted() }
    /// Returns `true` if the material is plaintext (dev/test only).
    pub fn is_plaintext(&self) -> bool { matches!(self, Self::Plaintext(_)) }
    /// Returns `true` if the key has been destroyed.
    pub fn is_destroyed(&self) -> bool { matches!(self, Self::Destroyed) }
...
pub struct KeyVersion {
    ...
    ///
    /// - `Encrypted(s)` → AES-GCM wrapped; `s` starts with `"enc:"`. Use when `CITADEL_MASTER_KEY` is set.
    /// - `Plaintext(s)` → raw hex-encoded bytes. **Development only.** Never acceptable in production.
