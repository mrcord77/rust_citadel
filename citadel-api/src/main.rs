...
fn hash_api_key(key: &str) -> [u8; 32] {
    ...
    // Use CITADEL_MASTER_KEY as HMAC key. Falls back to a fixed string only
    // because create_keystore() already enforces CITADEL_MASTER_KEY at startup;
    // the fallback fires only in the hash_apikey utility if run without the var.
    // Decode the 64-char hex string to actual 32-byte key material.
    // Using the raw ASCII hex string as HMAC key would work (server and tool match)
    // but wastes entropy — 64 low-entropy ASCII bytes instead of 32 key bytes.
    let master_key_str = std::env::var("CITADEL_MASTER_KEY")
        .unwrap_or_else(|_| "citadel-api-pepper-not-configured".to_string());
    let master_key_bytes = match hex::decode(master_key_str.trim()) {
        Ok(bytes) => bytes,
        Err(_) => master_key_str.into_bytes(),
    };
...
async fn auth_middleware(
    ...
    if store.keys.is_empty() {
        ...
        tracing::error!(
            path = %path,
            "CITADEL_API_KEY_HASH is not configured — rejecting request to protected endpoint. \
             "Set this environment variable to an HMAC-SHA256 hex hash of your API key."
        );
        return (
...
async fn encrypt_data(
    ...
    match state
        .keystore
        .encrypt(&KeyId::new(&id), req.plaintext.as_bytes(), &aad, &ctx)
        .await
    {
...
fn create_keystore(data_dir: &str) -> Keystore {
    ...
    // Without CITADEL_MASTER_KEY, secret key material is stored as plaintext hex
    // on disk — unacceptable for any environment handling real cryptographic keys.
    // The server will not start unless either:
    //   (a) CITADEL_MASTER_KEY is set to a 64-char hex string, OR
    //   (b) CITADEL_ALLOW_PLAINTEXT_KEYS=1 is explicitly set (development only).
    if std::env::var("CITADEL_MASTER_KEY").is_err()
        && std::env::var("CITADEL_ALLOW_PLAINTEXT_KEYS").as_deref() != Ok("1")
    {
        eprintln!();
        eprintln!("[FATAL] CITADEL_MASTER_KEY is not configured.");
        eprintln!("  Secret keys would be stored as plaintext on disk.");
        eprintln!("  This is not acceptable in any environment with real key material.");
        eprintln!();
        eprintln!("  To fix (generate a key and set it):");
        eprintln!("    openssl rand -hex 32");
        eprintln!("    export CITADEL_MASTER_KEY=<64-char hex output>");
        eprintln!();
        eprintln!("  To opt in to plaintext storage for local development only:");
        eprintln!("    export CITADEL_ALLOW_PLAINTEXT_KEYS=1");
        eprintln!();
        std::process::exit(1);
    }
    // ─────────────────────────────────────────────────────────────────────────
...
async fn seed_demo_keys(ks: &Keystore) {
    ...
    for i in 1..=4 {
        ...
        for _ in 0..i {
            let _ = ks.encrypt(&dek, b"demo payload", &aad, &ctx).await;
        }
    }
    let old = ks
        .generate(
    ...
    if let Some(hash_bytes) = resolve_bootstrap_hash() {
        ...
        tracing::warn!(
            "CITADEL_API_KEY_HASH is not set — no API credentials are configured. \
             ...
             "Set CITADEL_API_KEY_HASH to an HMAC-SHA256 hex hash (run: hash-apikey <key>)."
        );
    }
...
async fn main() {
    ...
    tracing::info!(
        ...
        rate_rps,
        rate_burst,
        "starting Citadel API Server v0.2.0"
    );
