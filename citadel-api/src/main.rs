...
fn create_keystore(data_dir: &str) -> Keystore {
    ...
    // Validate CITADEL_MASTER_KEY at startup: check existence, valid hex, and
    // correct length. A misconfigured key that passes only an existence check
    // causes runtime failures on every key operation instead of a clear startup
    // error. Fail fast with a specific diagnostic.
    match std::env::var("CITADEL_MASTER_KEY") {
        Ok(val) => {
            match hex::decode(val.trim()) {
                Err(e) => {
        ...
        Err(_) => {
            ...
            // Plaintext dev mode is active — hard block if CITADEL_ENV=production.
            if matches!(
                std::env::var("CITADEL_ENV").as_deref(),
                Ok("production") | Ok("prod")
            ) {
                eprintln!("[FATAL] CITADEL_ALLOW_PLAINTEXT_KEYS=1 with CITADEL_ENV=production.");
                eprintln!("  Plaintext key storage is not permitted in production.");
                eprintln!("  Set CITADEL_MASTER_KEY to a 64-char hex string.");
                std::process::exit(1);
            }
            tracing::warn!("CITADEL_MASTER_KEY not set. Plaintext key storage active (dev mode).");
        }
    }
    // ─────────────────────────────────────────────────────────────────────────
