...
# API note: key list returns field "id" (not "key_id")
#           key list returns key_type "DataEncrypting" (not "DEK")
#           key list returns state "ACTIVE" (uppercase)
#           create-key returns field "key_id"
# ---------------------------------------------------------------
...
if (-not $dek) {
    ...
    $dekId = $newId
    Log "BASELINE" "Created and activated DEK: $dekId" "INFO"
}
else {
    $dekId = $dek.id
...
if ($enc.Status -eq 200 -and $enc.Body.ciphertext_hex) {
    ...
    Log "ENCRYPT" "Encryption failed (status $($enc.Status)): $($enc.Body.error)" "FAIL"
}
...
$critScore = [double]$threat.score
Log "ESCALATE" "Threat score: $critScore, Level: $($threat.level) ($($threat.name))" "INFO"

if ($threat.level -ge 5) {
    Log "ESCALATE" "Reached CRITICAL (level 5)" "PASS"
}
elseif ($threat.level -ge 4) {
    Log "ESCALATE" "Reached HIGH (level 4) -- close to CRITICAL" "PASS"
}
else {
    Log "ESCALATE" "Did not reach CRITICAL -- got level $($threat.level)" "WARN"
}
...
# Search project directory only (not entire C: drive)
$found = Get-ChildItem -Path $projectDir -Filter $auditFile -Recurse -Depth 3 -ErrorAction SilentlyContinue | Select-Object -First 1
if ($found) {
...
if ($foundPath) {
    ...
    Log "CHAIN" "Check: did you restart server with the new main.rs?" "INFO"
}
Write-Host ""
