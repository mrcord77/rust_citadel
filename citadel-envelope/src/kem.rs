...
impl KemProvider for HybridX25519MlKem768Provider {
    ...
    fn encapsulate(pk: &PublicKey) -> Result<(Vec<u8>, Vec<u8>), EncodingError> {
        ...
        // Use a Zeroizing stack array so the 64-byte IKM is wiped on drop,
        // regardless of how the caller handles the returned Vec.
        ...
        combined_raw[SHARED_SECRET_BYTES..].copy_from_slice(mlkem_ss.as_slice());
