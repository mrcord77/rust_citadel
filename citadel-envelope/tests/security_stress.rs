...
fn context_isolation() {
    ...
    assert!(cit.open(&sk, &ct_med, &aad, &Context::raw(b"medical")).is_ok());
    assert!(cit.open(&sk, &ct_fin, &aad, &Context::raw(b"financial")).is_ok());
}
...
fn volume_10k_roundtrips() {
    ...
    assert!(elapsed < Duration::from_secs(60), "10k roundtrips took too long");
}
